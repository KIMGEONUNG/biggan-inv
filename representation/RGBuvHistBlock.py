...
class RGBuvHistBlock(nn.Module):
    def __init__(self, h=64, insz=150, resizing='interpolation',
                 ...
                 hist_boundary=None, green_only=False, device='cuda:1'):
            ...
            if not self.green_only:
                ...
                diff_v0 = abs(
                    Iv0 - torch.unsqueeze(torch.tensor(np.linspace(
                        ...
                        dim=0).to(self.device))
                if self.method == 'thresholding':
                    diff_u0 = torch.reshape(
                        diff_u0, (-1, self.h)) <= self.eps / 2
                    diff_v0 = torch.reshape(
                        diff_v0, (-1, self.h)) <= self.eps / 2
            ...
            if not self.green_only:
                ...
                diff_v2 = abs(
                    Iv2 - torch.unsqueeze(torch.tensor(np.linspace(
                        ...
                        dim=0).to(self.device))
                if self.method == 'thresholding':
                    diff_u2 = torch.reshape(
                        diff_u2, (-1, self.h)) <= self.eps / 2
                    diff_v2 = torch.reshape(
                        diff_v2, (-1, self.h)) <= self.eps / 2
                elif self.method == 'RBF':
...
if __name__ == '__main__':
    ...
    def load_img_as_tensor(path, dev):
        im = Image.open(path)
        x = ToTensor()(im).unsqueeze(0).to(dev) 
        return x

    dev = 'cuda:1'

    block = RGBuvHistBlock(h=64, insz=250, device=dev)
    x_source = load_img_as_tensor('./samples/sample02.jpg', dev)
    x_source = Variable(x_source, requires_grad=True)
    x_target = load_img_as_tensor('./samples/sample01.jpg', dev)

    # Optimizer
    optimizer = optim.Adam([x_source])
    ...
    for i in tqdm(range(10000)):
        ch_src = block(x_source)
        ch_tgt = block(x_target)

        loss = loss_fn(ch_src, ch_tgt)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        log.append(loss.item())
        if i % 100 == 0:
            y = x_source.squeeze(0)
            y = y - y.min()
            y = y / y.max()
            im = ToPILImage()(y)
            im.save('./outs/%05d.jpg' % i)

    plt.plot(log)
    plt.savefig('./outs/log.jpg')
