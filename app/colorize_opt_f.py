...
def parse():
    ...
    parser.add_argument('--num_layer_f', type=int, default=2)
    ...
    parser.add_argument('--feat_mask', action='store_true')
...
def main(args):
    ...
    if args.loss == 'lpips':
        ...
        loss_fn = VGGFeatLoss(args.path_vgg, 
                              idx_targets=args.vgg_target_layers).to(dev)
    else:
        raise Exception()
    ...
    for i, mask in enumerate(masks):
        ...
        with torch.no_grad():
            ...
            f = EG.E(x_resize, c_embd) 
            f = EG.G.forward_from_to(z, c_embd, EG.id_mid_layer, 
                                     args.num_layer_f, f)
            output = EG.G.forward_from(z, c_embd, args.num_layer_f, f)
        ...
        if args.feat_mask:
            m4f = transforms.Resize(f.shape[-2:], Image.LINEAR)(m)

            if False:
                m4f = m4f.sum(1).squeeze()
                m4f[m4f > 0] = 1
                plt.imshow(m4f.cpu())
                plt.show()
                exit()
            m4f = m4f.sum(1).repeat(1, 768, 1, 1)
            f_init = f.clone().detach()

        f = Variable(f, requires_grad=True)
        optimizer = optimizer([f], lr=args.lr)
        ...
        for j in tbar:
            ...
            # trivial shape colorection
            m_resize = transforms.Resize(output.shape[-2:])(m)

            if False: # Try 1
                m_sum = m_resize.sum(1).repeat(1, 3, 1, 1)
                output_fore[m_sum == 0] = 0

            if True: # Try 2
                m_sum = m_resize.sum(1).repeat(1, 3, 1, 1)
                m_resize[m_sum == 0] = output_init[m_sum == 0]

            # if j == 2:
            if False:
                im1 = ToPILImage()(output.squeeze())
                im2 = ToPILImage()(output_fore.squeeze())
                im3 = ToPILImage()(m_resize.squeeze())
                plt.subplot(1, 3, 1)
                plt.imshow(im1)
                plt.title('original')
                plt.subplot(1, 3, 2)
            ...
            if j % args.interval == 0:
                if args.feat_mask:
                    f_copy = f.clone().detach()
                    f_copy[m4f == 0] = f_init[m4f == 0]
                    with torch.no_grad():
                        output = EG.G.forward_from(z, c_embd, args.num_layer_f, f_copy)
                        output = output.add(1).div(2)
